...
Manuais canônicos aplicáveis: [[CONTRACT]] (código), [[TESTES]] (qualidade), [[AGILE]] (cadência), [[DEPLOY_CHECKLIST]] (deploy), [[AUDITORIAS]] (checklists). Hub: [[Standards]].
...
Quem lidera cada fase (detalhe e RACI completo em [[ORG]] secao 4):
...
| 10. GTM | Camilo (CMO) | product-marketing-manager, growth-engineer, content-seo, pr-comms, community-manager |
...
### 2.5. Entregáveis
PRD aprovado, roadmap, backlog inicial (GitHub Projects).
...
### 4.6. Infraestrutura
Cloud (AWS/GCP/Azure) ou VPS (Hetzner, Hostinger) ou shared hosting. IaC (Terraform, OpenTofu, Pulumi), containers e orquestração só se necessário, CDN.

### 4.7. ADRs (Architecture Decision Records)
Documento curto por decisão: contexto, decisão, consequências, alternativas. Versionado. Skill `create-adr`.

### 4.8. Entregáveis
Diagrama C4, ADRs iniciais, spec OpenAPI/GraphQL, ERD, threat model.

### 4.9. Liderança
Caetano (CTO), via `software-architect` (modo colaborativo: opções antes de gravar canônico) e `security-engineer`. Apoio: Narciso (CISO). Papéis humanos: Tech Lead, Solution Architect, Security Engineer, DBA, SRE.
...
### 5.2. CI/CD
CI: lint, type-check, testes, build, SAST, análise de licenças. CD: deploy automático para staging no merge, promoção para produção. Ferramentas: GitHub Actions.
...
Múltiplas trilhas em paralelo, sincronizadas por Cosmo (COO) via daily standups, sprint planning (Scrum) ou cadências leves (Shape Up, Kanban). Ver [[AGILE]].
...
Pirâmide ou troféu (Kent C. Dodds). Detalhe em [[TESTES]].
...
### 8.6. Liderança
Narciso (CISO) lado técnico via `security-engineer`; Cláudio (CLO) lado jurídico via `compliance-legal`. Papéis humanos: Security Engineer, Pentester, DPO, Legal Counsel, Compliance Officer. Skill `security-threat-model`.
...
### 10.8. Liderança
Camilo (CMO), via `product-marketing-manager`, `growth-engineer`, `content-seo`, `pr-comms`, `community-manager`. Receita B2B com Cícero (CRO), pricing com Confúcio (CFO). Papéis humanos: CMO, PMM, Content Marketer, Growth Engineer, PR Specialist, Designer de marketing, Community Manager.
...
Ver [[DEPLOY_CHECKLIST]] para as 7 fases de deploy irreversível.
...
Cadência regular (semanal, bi-semanal), sprints de tech debt, auditorias periódicas (segurança trimestral, acessibilidade semestral, ver [[AUDITORIAS]]).
...
Quem decide e re-avalia: **Cósimo (Chief of Staff)** (agent `cosimo-chief-of-staff`). Critérios em [[lideranca_pipeline_release]] secao 5. Detalhe em [[ORG]] secao 5.

| Variante | Porte | Como o pipeline muda |
|---|---|---|
| **Pipeline-Sprint** | early minimalista / projeto pequeno | Fases 0-2 viram um one-pager; 3 vira wireframe; 4 vira 3 ADRs; 6-8 contínuos; 9-12 enxutos. Só Celso e Caetano ativos. Sem cerimônia. |
...
Criticidade sobrepõe headcount: projeto pequeno que toca prontuário médico, dinheiro ou PII mantém Narciso (CISO) e Cláudio (CLO) ativos mesmo no Pipeline-Sprint.
...
|---|---|---|
| Liderança/Produto | CEO, CTO, CPO, PM, PMM, Product Owner, Business Analyst | celso-ceo, caetano-cto, capitolino-cpo, product-manager, business-analyst |
| Design | UX Researcher, UX/UI Designer, UX Writer, Design System Eng, Acessibilidade, Motion | ux-researcher, ux-ui-designer, ux-writer, accessibility-specialist |
...
| Marketing/Growth | CMO, Growth Eng/PM, Content/SEO, PR, Community | camilo-cmo, product-marketing-manager, growth-engineer, content-seo, pr-comms, community-manager |
...
| Gestão | Engineering Manager, Scrum Master, Producer, Release Manager | engineering-manager, scrum-master, game-producer, engineering-coach, release-manager |
...
| Categoria | Opções |
|---|---|
| Gestão de produto | GitHub Projects |
...
11. **Over-engineering**: aplicar pipeline bigtech a projeto pequeno/early (combatido por Cósimo).
...
**Total típico para SaaS pequeno: 6 a 14 meses** da ideia ao 1.0. No Pipeline-Sprint (early minimalista), pode cair para semanas.

---

## Links

- [[ORG]] (constelação, RACI, pendências) · [[lideranca_pipeline_release]] (teoria C-level)
- [[Standards]] · [[CONTRACT]] · [[TESTES]] · [[AGILE]] · [[DEPLOY_CHECKLIST]] · [[AUDITORIAS]]
- [[CLAUDE]] · [[TODO]] · [[Projects]]
