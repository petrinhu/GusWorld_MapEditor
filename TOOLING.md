...
Hub: [[Standards]]. Irmãos: [[CONTRACT]], [[TESTES]], [[AGILE]], [[DEPLOY_CHECKLIST]], [[AUDITORIAS]]. Organização: [[ORG]].

## Legenda de status

- ✓ **temos** (instalado no sistema)
- ⬇ **baixar** (faltando, instalar sob demanda com o comando da coluna)
- ↺ **preferir** (temos o moderno; usar no lugar do legado)

Regra: o agent usa a ferramenta canônica do seu domínio SEMPRE que aplicável (não reinventa em shell cru). Se faltar (⬇), instala com o comando aqui antes de usar. Respeitar limites de hardware ([[feedback_hardware_resource_limits]]) e prioridade MCP ([[feedback_mcp_priority]]).
...
| strace / ltrace | ✓/⬇ | trace de syscalls/libcalls | performance, linux-diag | `sudo dnf install strace ltrace` |

---

## 4. Segurança (security-engineer, network-security-engineer, internal-auditor, Narciso/CISO)
...
## 7. Dados (data-engineer, data-scientist, ml-engineer, Cândido/CDO)
...
| ollama | ✓ | LLM local (respeitar VRAM 4GB) | (instalado) |
...
| Ferramenta | Status | Para quê | Agents | Instalar |
|---|---|---|---|---|
| scancode-toolkit | ⬇ | scan de licença OSS | compliance-legal, Cláudio/CLO, auditor | `pipx install scancode-toolkit` |
...
| hledger | ⬇ | contabilidade texto-plano | Confúcio/CFO | `sudo dnf install hledger` |
| gh | ✓ | CLI GitHub | todos | `sudo dnf install gh` |
...
| linux-diag | strace, ss, dig, tcpdump, lynis (read-only, NUNCA modifica) |

---

## 12. Resumo de status

- **Já temos (✓):** git, gh, rg, fd, fzf, jq, ast-grep, cmake, ninja, make, gcc/clang, clang-format/tidy, cppcheck, ctest, playwright, gitleaks, nuclei, nmap, openssl, lynis, chkrootkit, sqlite3, graphviz, perf, valgrind, heaptrack, strace, ffmpeg, imagemagick, ollama, python3/pipx/uv, wget, tcpdump, tshark, dig, nft, httpie, ss, arp-scan, ipcalc, docker, podman, npm, cargo, go.
- **Preferir (↺):** rg sobre grep, fd sobre find, delta sobre diff, nft sobre iptables, uv/pipx sobre pip global, biome sobre eslint+prettier, ruff sobre flake8+black, tokei sobre cloc, tofu sobre terraform.
- **Baixar (⬇):** sob demanda, comando na linha de cada ferramenta. Tier 1 sugerido (mais usado): ruff, shellcheck, shfmt, yamllint, vale, semgrep, trivy, syft, pandoc, mermaid-cli, duckdb, yq, miller, hyperfine, delta, pa11y, lighthouse, restic, parallel, entr, tokei.

---

## Links

- [[Standards]] · [[ORG]] · [[CONTRACT]] · [[TESTES]] · [[AGILE]] · [[DEPLOY_CHECKLIST]] · [[AUDITORIAS]]
- [[CLAUDE]] · [[TODO]] · [[pipeline_release_1.0]]
