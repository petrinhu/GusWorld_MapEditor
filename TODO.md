...
- **Os itens `COR-10`, `COR-11` e `FMT-7` foram REPONTUADOS pelas lentes em 24/08/2026**, cumprindo a dívida que esta seção declarava. Eles tinham nascido com número do orquestrador, e o Chief of Staff recusou reordenar com eles porque isso seria o orquestrador auto-validando a própria pontuação. **A lente de produto achou um erro de método, não de calibragem:** o número do `FMT-7` estava contaminado pela POSIÇÃO do item na tabela, quando a nota é justamente o que deveria decidir a posição. **A lente de engenharia subiu o `COR-11` de 3 para 5** por causa de uma decisão de produto em aberto (ver os achados abaixo). Agora os 34 itens têm número de lente.
...
- **`COR-11` tem uma decisão de produto em aberto que muda o tamanho da fatia**, achada pela lente de engenharia em 24/08/2026: ao redimensionar um mapa, qual ponto fica fixo (cresce a partir de qual canto ou do centro?), e o que acontece com objeto ou volume que fica **fora** do novo limite ao encolher (recusar, cortar, empurrar para dentro?). Sem essa decisão o implementador para no meio da fatia para perguntar, e fatia interrompida custa mais que fatia maior planejada. **Vai ao líder.**
...
| 12.0 | COR-1 | W1 | Núcleo | Scaffolding CMake C++23 e harness de teste rodando nos cinco alvos. Destrava literalmente todo o resto do núcleo. | Alta | DEC-1, DEC-2 | Média | 🔍 Pendente verificação | — |
...
| 4.2 | COR-10 | W3 | Núcleo | Herança de tipo com marca de sobrescrita por instância, chaveada por UUID de instância. **É trabalho nosso** por decisão do GlintFx (item 7): o formato guarda instâncias resolvidas mais referência de tipo opaca, e a herança fica no editor. Sem a marca, ao reabrir não se sabe se o valor foi escolhido pelo autor ou herdado, e a próxima mudança de tipo sobrescreve o trabalho dele. | Alta | COR-2 | Média | ⏳ Pendente | — |
| 2.6 | COR-11 | W3 | Núcleo | Recálculo de redimensionamento de mapa. **É trabalho nosso** por decisão do GlintFx (item 8): a lib só fixa a convenção de coordenada na especificação, porque redimensionar é operação de autoria e não de mecanismo. Sem isto, crescer o mapa desloca objeto para fora sem o autor perceber. | Média | COR-2 | Média | ⏳ Pendente | — |
...
| 1.7 | FMT-7 | W7 | Formato | Agrupamento de objetos da sessão de edição, no nosso arquivo ao lado. **Fica fora do formato** por decisão do GlintFx (item 9): grupo de sessão é do editor; grupo que o jogo precisa em runtime vai no canal de propriedades deles. | Baixa | FMT-5 | Média | ⏳ Pendente | — |
| 8.0 | FMT-1 | W8 | Formato | Abrir mapa pelo leitor público do GlintFx. Primeira vez que o editor toca um mapa real: valida ou derruba tudo que COR-2 supôs. Dispara a `L-02`. | Alta | EXT-1, COR-2 | Média | ⏳ Pendente | — |
| 9.0 | FMT-6 | W9 | Formato | Validação de teleporte entre mapas: o destino aponta para um mapa que existe. Viável porque o destino é endereço direto por UUID de mapa. | Média | FMT-1 | Baixa | ⏳ Pendente | — |
| 5.2 | FMT-2 | W9 | Formato | Gravar pelo escritor público do GlintFx. Fecha o ciclo abrir, editar, salvar. Sai errado em silêncio se COR-7 não estiver pronto e testado antes. | Alta | FMT-1, COR-7 | Média | ⏳ Pendente | — |
| 7.0 | HL-1 | W10 | Headless | **Modo headless**: abrir, validar, transformar e gravar mapa por linha de comando, sem janela (`L-16`). Depende do formato, **não** da janela. É o que torna o papel de implementador de referência verificável, porque permite exercitar o formato no CI dos cinco alvos a cada commit. | Alta | FMT-2, COR-9 | Média | ⏳ Pendente | — |
| 4.2 | FMT-3 | W10 | Formato | Preservar bloco desconhecido ao regravar, honrando o bit "seguro de copiar". Exigência normativa do formato, conquistada por pedido nosso. Sem ela, salvar num editor desatualizado destrói dado alheio sem aviso. | Alta | FMT-2 | Média | ⏳ Pendente | — |
| 8.0 | FMT-4 | W11 | Formato | Portão de ida e volta byte a byte: abrir, salvar sem mudar nada, comparar. A `L-09` exige que nasça junto com a leitura e a escrita, não depois. Roda **por dentro do modo headless**, nos cinco alvos. | Alta | FMT-1, FMT-2, HL-1 | Média | ⏳ Pendente | — |
| 9.7 | APP-1 | W12 | Aplicação | Janela e laço principal sobre a API do GlintFx. Primeira vez que o editor existe como programa visível. | Alta | EXT-2 | Média | ⏳ Pendente | — |
| 0.8 | DES-1 | W12 | Design | Desenho da tela em alta fidelidade (o WYSIWYG). Espera o GlintFx ter janela, por ordem do líder e contra a recomendação registrada. Enquanto não for feito, não temos a lista de recursos de interface a pedir a eles. | Média | EXT-2 | Alta | 🎨 Pendente design | — |
| 6.3 | APP-2 | W13 | Aplicação | Viewport da grade com câmera enquadrando o conteúdo. Armadilha conhecida: câmera padrão centra a origem do mundo, não o conteúdo. | Alta | APP-1, DES-1 | Média | ⏳ Pendente | — |
| 2.5 | DEC-3 | W13 | Decisão | Menu de transformação do objeto na interface: rotação, aumento, redução. Pode esperar sem custo porque não existe tela onde desenhá-lo. | Baixa | DES-1 | Baixa | 🎨 Pendente design | — |
| 5.2 | APP-3 | W14 | Aplicação | Pintura de terreno na grade, uma camada (`L-14`). Primeiro fluxo interativo de ponta a ponta: une câmera, entrada, comando e transação num só gesto. | Alta | APP-2, COR-4 | Média | ⏳ Pendente | — |
| 4.2 | APP-7 | W14 | Aplicação | Painéis (inspetor, paleta, lista de objetos) assinando o sinal do histórico, sem cópia própria de estado. Implementado errado uma vez, é difícil de erradicar: o defeito documentado aparece na seleção múltipla. | Alta | APP-2, COR-3 | Média | ⏳ Pendente | — |
| 5.2 | APP-4 | W15 | Aplicação | Posicionar objeto e transformá-lo: rotação e escala em ponto flutuante, conforme o formato aceita. | Alta | APP-3, DEC-3 | Média | ⏳ Pendente | — |
| 3.6 | APP-5 | W15 | Aplicação | Edição dos seis tipos de volume da `L-14`, com lista de volumes por objeto. O formato guarda instâncias resolvidas mais referência de tipo opaca; a herança de tipo é nossa e vive no `COR-10`. | Alta | APP-3 | Alta | ⏳ Pendente | — |
| 3.0 | APP-6 | W15 | Aplicação | Abas multi-mapa com histórico único compartilhado, e troca automática de aba quando o desfazer alterar um mapa que não é o da aba ativa. A troca é mitigação **obrigatória** pela `L-14`. | Alta | APP-3, COR-3 | Alta | ⏳ Pendente | — |
| 4.3 | APP-8 | W16 | Aplicação | Exibir o diagnóstico do GlintFx (escala desigual em forma redonda) na interface **antes** de salvar, enquanto o autor ainda pode consertar. | Média | APP-5 | Média | ⏳ Pendente | — |
| 2.6 | TST-1 | W17 | Testes | Testes não unitários conforme o `TESTES.md` deste projeto: estática, propriedade, sanitizers, segredos, CVE, ida e volta, espelho local de CI e headless, nos cinco alvos. Teste unitário não é item: anda dentro da fatia. | Alta | APP-8, FMT-4 | Alta | ⏳ Pendente | — |
| 5.3 | AUD-1 | W18 | Auditoria | Auditorias conforme o `AUDITORIAS.md` deste projeto: fronteira de camadas, integridade do arquivo gravado, histórico persistido, fronteira com o GlintFx, escopo. Por `L-06`, C-level em fable, e implementador nunca audita o próprio trabalho. | Alta | TST-1 | Média | ⏳ Pendente | — |
| 0.8 | WIKI | W19 | Documentação | Wiki do repositório mais documentação extensa em registro didático para iniciante em computação, explicando todo jargão sem assumir conhecimento. Item fixo de fim de tabela. | Baixa | AUD-1 | Alta | ⏳ Pendente | — |
